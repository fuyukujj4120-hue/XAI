...
家貓情緒標註研究系統
版本 A：無部位特徵提示
流程：受試者基本資料 → 照片標註（直覺式情緒判斷）→ 整體反饋 → 完成

本版依版本 B 架構整理：
- 使用 annotations/ 作為標註照片資料夾
- 使用 imgaes/ 作為情緒定義圖片資料夾
- 保留 sidebar、手機可收合設定、Google Sheet 同步與單張標註時間紀錄
- 為了與版本 B 的 Google Sheet 欄位一致，特徵欄位與 prompt_helpfulness 會保留但填空
"""
...
# 與版本 B / Google Sheet 欄位對齊：版本 A 不使用特徵欄位與 prompt_helpfulness，會填空。
DATA_COLUMNS = [
    "annotator_id",
...
@media (max-width: 768px) {
    section[data-testid="stSidebar"] { background: #e8f4fd !important; }
    section[data-testid="stSidebar"] img { max-height: 48vh; }
    .main-title { font-size: 21px; }
    .sub-title, .info-card { font-size: 13px; }
}
            ...
            function t(){
                try{
                    ...
                    ['section.main','main','.stApp'].forEach(function(s){
                        window.parent.document.querySelectorAll(s).forEach(function(el){el.scrollTop=0;});
                    });
                }catch(e){}
            }
...
# ─────────────────────────────────────────────
def render_intro():
    st.markdown('<div class="version-badge">版本 A</div>', unsafe_allow_html=True)
    st.markdown('<div class="main-title">🐱 家貓情緒標註研究</div>', unsafe_allow_html=True)
    st.markdown('<div class="sub-title">版本 A：請直接觀看照片，依照您的直覺與觀察選擇情緒類別。</div>', unsafe_allow_html=True)
...
def render_sidebar():
    with st.sidebar:
        ...
        st.markdown(
            '<div class="version-badge" style="margin-bottom:10px;">版本 A</div>',
            unsafe_allow_html=True,
        )
        st.markdown("### 家貓照片")
...
def render_task():
    ...
    st.markdown(
        f"""
        <div class="info-card">
            請直接觀看左側照片，依照您的直覺與觀察選擇最符合的情緒。<br><br>
    ...
    # Step 2：信心與難度
    st.markdown("## Step 2：填寫標註信心與判斷難度")
